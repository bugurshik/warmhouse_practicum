...
Bounded Contexts:
* Контекст: Управление устройствами (датчики как устройства).
...
```markdown
[Тёплый дом - Контекст]("./AS_IS_Context_Diagram.png")
```
...
Для взаимодействия между микросервисами выбран тип REST API.

- Гибкость, общеизвестный стандарт, простота интеграций а также легкое документирование и отладка фактические делают REST API выбором по умолчанию.

- С ростом проекта рекомендуется плавный переход на gRPC взаимодействия между микросервисами используя комбинированный подход (какое-то время использовать REST + gRPC)
